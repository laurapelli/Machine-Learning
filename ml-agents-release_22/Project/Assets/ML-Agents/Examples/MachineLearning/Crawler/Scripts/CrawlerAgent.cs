...
public class CrawlerAgent : Agent
{
    ...
    {
        SpawnTarget(TargetPrefab, transform.position, obstacles); //spawn target
    ...
    void SpawnTarget(Transform prefab, Vector3 pos, Transform[] obstacles)
    {
        // Check the obstacles positions
        foreach (Transform obstacle in obstacles)
        {
            if (Vector3.Distance(pos, obstacle.position) < 0.1f)
            {
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        {
            Vector3 localPos = m_OrientationCube.transform.InverseTransformPoint(obstacle.position);
            sensor.AddObservation(localPos / 10f); // normalize
        }
    ...
    void FixedUpdate()
    {
        ...
        AddReward(matchSpeedReward * lookAtTargetReward * 2);
    }
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        {
            AddReward(-2f); // Penalize in case it collision with an obstacle
        }
    }
}
